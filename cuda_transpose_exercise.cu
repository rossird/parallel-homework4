...
{
  int thread_2D_pos_x =	blockIdx.x * blockDim.x + threadIdx.x;
  int thread_2D_pos_y = blockIdx.y * blockDim.y + threadIdx.y;
  
  if (thread_2D_pos_x >= N || thread_2D_pos_y >= N)
    return;
    
  out[thread_2D_pos_x  + thread_2D_pos_y*N] = in[thread_2D_pos_y + thread_2D_pos_x *N]; // out(j,i) = in(i,j)
...
transpose_parallel_per_element_tiled_shared(float in[], float out[])
{
  ...
  //First thread in each block will copy the data into shared memory
  if (threadIdx.x == 0 && threadIdx.y == 0) {
    for(int x = 0; x < N; x++) {
      for(int y = 0; y < N; y++) {
        blockData[x+N*y] = in[blockStart + x + N*y];
      }
    }
  }
  __syncthreads();
    
  out[thread_2D_pos_x  + thread_2D_pos_y*N] = blockData[threadIdx.y + threadIdx.x * N];
}
...
int main(int argc, char **argv)
{
	...
	size_t sharedMemSize = N * N;
     timer.Start();
	transpose_parallel_per_element_tiled<<<blocks_tiled_sh,threads_tiled_sh, sharedMemSize>>>(d_in, d_out);
	timer.Stop();
    for (int i=0; i < N*N; ++i){out[i] = 0.0;}
	cudaMemcpy(out, d_out, numbytes, cudaMemcpyDeviceToHost);
